...
module.exports = (cli, denon) => {
  ...
  cli.command('volume?').action(()=> denon.command('MV?'));
  cli.command('alarm?').action(()=> denon.command('TO?').then(function(value){
	  cli.log('Query sent');
	  cli.log(value);
  }));
  cli.command('alarmon').action(()=> denon.command('TSEVERY 20740-20840 FA01 12 1'));
  cli.command('alarmoff').action(()=> denon.command('TSEVERY 20740-20840 FA01 12 0'));
